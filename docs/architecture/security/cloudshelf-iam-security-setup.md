...
│                                                                     │
│  ┌───────────────┐    ┌───────────────┐    ┌───────────────┐        │
│  │   📋 POLICIES │────│   👤 ROLES    │────│  🛡️ GROUPS   │        │
│  │               │    │               │    │               │        │
│  │ • RDS Access  │    │ • Lambda Exec │    │ • Lambda SG   │        │
│  │ • DynamoDB    │    │ • API Gateway │    │ • RDS SG      │        │
│  │ • Lambda Inv  │    │ • RDS Access  │    │ • VPC Rules   │        │
│  │ • S3 Assets   │    │               │    │               │        │
│  └───────────────┘    └───────────────┘    └───────────────┘        │
│         │                      │                      │             │
└─────────┼──────────────────────┼──────────────────────┼─────────────┘
...
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  ⚡ LAMBDA FUNCTIONS           📊 DATABASES              🌐 GATEWAY  │
│  ┌─────────────────┐          ┌─────────────────┐      ┌──────────┐ │
│  │  📚 Book        │─────────▶│  🗄️ PostgreSQL │      │ 🔌 API   │ │
│  │    Catalog      │          │     (RDS)       │      │  Gateway │ │
│  │                 │          │                 │      │          │ │
│  │  🛒 Shopping    │─────────▶│  ⚡ DynamoDB    │      │          │ │
│  │    Cart         │          │   (NoSQL)       │      │          │ │
...
SECURITY FLOW:
1. 📋 Custom Policies → Define specific permissions
2. 👤 IAM Roles → Assume policies for service access
3. 🛡️ Security Groups → Network-level access control
4. ⚡ Lambda Functions → Execute with assigned roles
5. 🔌 API Gateway → Invoke functions via resource policies
```
